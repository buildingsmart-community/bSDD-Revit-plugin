...
namespace BsddRevitPlugin.Logic.UI.Wrappers
{
    ...
    public class SelectElements
    {
        ...
        public static List<IfcPropertySet> IfcDefinition(Document doc, Element elem)
        {
            ...
            foreach (IfcPropertySingleValue PropValue in hasProperties)
            {
                if (PropValue.Name.StartsWith("bsdd/prop/", true, null) == true)
                {
                    promptArr = PropValue.Name.Remove(0, 10).Split('/');
                    if (!isDefinedBy.Any(ps => ps.Name == promptArr[0]))
                    {
                        IfcPropertySet ifcPropSet = new IfcPropertySet();
                        ifcPropSet.Type = "IfcPropertySet";
                        ifcPropSet.Name = promptArr[0];
                        ifcPropSet.HasProperties = new List<IfcProperty>();
                    ...
                    else
                    {
                        ...
                        {
                            if (ifcPropSet.Name == promptArr[0])
                            {
                                ifcPropSet.HasProperties.Add(PropValue);
                            }
                        }
            ...
            //remove bsdd/prop/pset from name
            foreach (IfcPropertySet pSet in isDefinedBy)
            {
                foreach (IfcProperty property in pSet.HasProperties)
                {
                    if (property.Name.StartsWith("bsdd/prop/", true, null) == true)
                    {
                        promptArr = property.Name.Remove(0, 10).Split('/');
                        property.Name = promptArr[1];
                    }
                }
            }
